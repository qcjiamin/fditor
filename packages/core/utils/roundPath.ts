...
export function roundPathCorners(pathData: TSimplePathData, radius: number): string {
  ...
  for (const sub of subPaths) {
    ...
    if (segs.length < 2) {
      sub.forEach((c: any) => { finalSVG += c.data.join(' ') + ' ' })
      continue
    }
    ...
    for (let k = 0; k < vertexCount; k++) {
      ...
      if (prevSeg.type === 'L') {
        ...
        tanIn = getCubicDerivative(prevSeg.start, new Point(prevSeg.data[1], prevSeg.data[2]), new Point(prevSeg.data[3], prevSeg.data[4]), prevSeg.end, 1)
      }
      ...
      if (currSeg.type === 'L') {
        ...
        tanOut = getCubicDerivative(currSeg.start, new Point(currSeg.data[1], currSeg.data[2]), new Point(currSeg.data[3], currSeg.data[4]), currSeg.end, 0)
      }
