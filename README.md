...
多选设置组合原则
lovart:
  完全相同的元素，多选时，才显示属性设置；2及2以上类型元素多选时，只显示多选属性
  组只显示多选属性
  多选时，以fill举例，fill不同时，属性设置组件上显示当前为多色状态，颜色相同时，显示改颜色
todo：
  多选属性条变为动态的，当多选类型唯一时，复用该类型属性条
  属性获取要兼容多选状态，返回的属性为数组，数组要去重，属性完全一致时，UI按单选显示结果
  属性设置组件要兼容多选状态
当前：
  多选和组统一只显示多选属性